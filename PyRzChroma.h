#ifndef _PYRZCHROMA_H_
#define _PYRZCHROMA_H_
...
public:
  ...
  void setKey(RZKEY2 key, float red, float green, float blue);
  void unsetKey(RZKEY2 key);
  void clearAllKeys();

  COLORREF getColor(float red, float green, float blue);

private:
  void applyKeyColors();

  HMODULE m_ChromaSDKModule;
  std::map<RZKEY2, COLORREF> _setKeyColors;
  COLORREF _staticColor;
};
